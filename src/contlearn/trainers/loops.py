...
class TrainingLoopsMixin:
    ...
    def train__CL__graph(self, train__, params, static,\
        ...
        for epoch in pbar:
            # Each epoch: full pass over both dataloaders
            trainiter = iter(trainloader)
            expiter = iter(exploader)
            for batch, batch_ex in zip(trainiter, expiter):
                batch = transforms(batch)
            ...
            if epoch % save_iter == 0 and epoch > 0:
...
# ---------------------------------------------------------------------
    ...
    def train__CL__reg(self, train__, params,\
        ...
        """Epoch-based regression training loop for continual learning.
        ...
        Args:
            ...
            static: Static model components
            optim: Optimizer
            n_iter: Number of epochs (full passes over dataset)
        ...
        trainloader, exploader, valloader, testloader = train__
        opt_state = optim.init(params)
        pbar = tqdm(range(n_iter))  # n_iter = number of epochs
        flag = config["flag"]

        for epoch in pbar:
            # Reset iterators at start of each epoch
            trainiter = iter(trainloader)
            expiter = iter(exploader)
            ...
            # Inner loop: full pass over both dataloaders (zip stops at smaller)
            for batch, batch_ex in zip(trainiter, expiter):
                (x, y) = batch
                (exp_x, exp_y) = batch_ex

                # Convert to numpy and align batch sizes
                ...
                epoch_dVstar_dtheta.append(float(dVstar_dtheta))
                epoch_dVstar_dx.append(float(dVstar_dx))
            ...
            if epoch % save_iter == 0 and epoch > 0:
                ...
                dVstar_dtheta_avg = np_.mean(epoch_dVstar_dtheta)
                dVstar_dx_avg = np_.mean(epoch_dVstar_dx)
                ...
                pbar.set_postfix_str(
                    f"MSE={V_avg:.6e} dV_dx={dVstar_dx_avg:.6e} dV_dθ={dVstar_dtheta_avg:.6e} "
                    f"H={H_avg:.6e} ||dH_dθ||={grad_norm_avg:.6e} | "
    ...
    def train__CL__class(self, train__, params,\
                    ...
                    save_iter=10, task_id=0,\
                    config={}, record_dict = {},notABTrain = True):
        """Epoch-based classification training loop for continual learning.
        ...
        for epoch in pbar:
            ...
            for batch, batch_ex in zip(trainiter, expiter):
                ...
                # Compute training metric (accuracy)
            ...
            if epoch % save_iter == 0 and epoch > 0:
                ...
                pbar.set_postfix_str(
                    f"CE={V_avg:.6e} dV_dx={dVstar_dx_avg:.6e} dV_dθ={dVstar_dtheta_avg:.6e} "
                    f"H={H_avg:.6e} ||dH_dθ||={grad_norm_avg:.6e} | "
                    f"Tr/Acc={train_metric_avg:.4f} Te/Cur={test_current_metric:.4f} Te/Exp={test_exp_metric:.4f}"
                )
