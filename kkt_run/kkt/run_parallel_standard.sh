#!/bin/bash
# Parallel job distributor for KKT server - Standard (non-AWB) configs only
# Runs: mnist, cifar10, cifar100, sine, synthetic_graph

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_DIR="${SCRIPT_DIR}/configs"
LOG_DIR="${SCRIPT_DIR}/logs"
SUMMARY_FILE="${LOG_DIR}/job_summary_standard.txt"
...
echo "" > "${SUMMARY_FILE}"
echo "==========================================================================================================" >> "${SUMMARY_FILE}"
echo "NEW RUN (STANDARD - No AWB): $(date)" >> "${SUMMARY_FILE}"
echo "==========================================================================================================" >> "${SUMMARY_FILE}"
echo "" >> "${SUMMARY_FILE}"

# Standard (non-AWB) config files only
CONFIGS=(
    "mnist.json"
    "cifar10.json"
    "cifar100.json"
...
echo "=========================================="
echo "Running STANDARD (non-AWB) configs"
echo "=========================================="
SKIPPED_COUNT=0
TO_RUN=()
...
if [ ${#CONFIGS[@]} -eq 0 ]; then
    echo "All standard configs already completed!"
    exit 0
fi
