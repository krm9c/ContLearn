...
class RecordingMixin:
    ...
    def _compute_eigenvalues(self, model, combined=False):
        """Compute eigenvalues of A and B matrices grouped by layer.
        ...
        try:
            # Check if model has A and B attributes (AWB-enabled models)
            if not hasattr(model, 'A') or not hasattr(model, 'B'):
                return eigenvalues

            # Handle different model types: MLP, CNN, CNN3D, myNN (GNN)
            # MLP has A, B as lists
            if hasattr(model, 'A') and isinstance(model.A, list):
                for i, (A_matrix, B_matrix) in enumerate(zip(model.A, model.B)):
                    # Compute eigenvalues for each layer's A and B matrices
            ...
            # CNN/CNN3D has A_conv, B_conv, A_feed, B_feed
            ...
            # myNN (GNN) has A_gcn, B_gcn, A_feed, B_feed
    ...
    def save_all_runs(all_runs_records: Dict[str, Any], base_path: str, config: Dict[str, Any]):
        ...
        # Create filename: problem_dataset_network_allruns.pkl
