#!/bin/bash
# Single MNIST experiment runner - runs one config with one run_id
# Usage: run_single.sh <config_file> <run_id> [gpu_id]
#
# Example: run_single.sh mnist_condition1_baseline.json 0 0
...
if [ -z "${CONFIG_FILE}" ] || [ -z "${RUN_ID}" ]; then
    ...
    echo ""
    echo "Example: run_single.sh mnist_condition1_baseline.json 0 0"
    exit 1
fi
...
# Extract condition name from config filename (e.g., "mnist_condition1_baseline" -> "condition1")
CONDITION=$(echo "${CONFIG_BASE}" | sed -E 's/mnist_//' | sed -E 's/_.*//')
