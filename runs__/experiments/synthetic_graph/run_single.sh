#!/bin/bash
# Single Synthetic Graph experiment runner - runs one config with one run_id
# Usage: run_single.sh <config_file> <run_id> [gpu_id]
#
# Example: run_single.sh synthetic_graph_2task_condition1_baseline.json 0 0
...
if [ -z "${CONFIG_FILE}" ] || [ -z "${RUN_ID}" ]; then
    ...
    echo ""
    echo "Example: run_single.sh synthetic_graph_2task_condition1_baseline.json 0 0"
    exit 1
fi
...
# Extract condition name from config filename
CONDITION=$(echo "${CONFIG_BASE}" | sed -E 's/synthetic_graph_2task_//' | sed -E 's/_.*//')
...
echo ""
echo "=========================================================================================================="
echo "SYNTHETIC GRAPH EXPERIMENT - NEW RUN"
echo "=========================================================================================================="
